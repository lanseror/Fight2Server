...
<table border="1" width="80%">
    ...
    <tr>
        ...
        <td><#if entry.isNpc()>NPC<#else>玩家</#if></td>
        <td>
        <#if entry.isDisabled()>
        ...
        <#if entry.isNpc()>
            <a href="<@s.url namespace="/user" action="notnpc"><@s.param name="id" value="${entry.id}" /></@s.url>">设置为玩家</a>
        <#else>
            <a href="<@s.url namespace="/user" action="setnpc"><@s.param name="id" value="${entry.id}" /></@s.url>">设置为NPC</a>
        </#if>
         &nbsp;&nbsp;|&nbsp;&nbsp;
